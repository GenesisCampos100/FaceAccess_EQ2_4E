...
# Formatos y textos visibles que se actualizan en pantalla
FORMATO_FECHA = "%d/%m/%Y"
FORMATO_HORA = "%H:%M"
FORMATO_FECHA_HORA = "%d/%m/%Y  %H:%M"
TEXTO_ROSTRO_NO_REGISTRADO = "Rostro no registrado"
TEXTO_SIN_COINCIDENCIA = "No se encontró coincidencia en el registro"

APP_W_OBJETIVO = 600
APP_H_OBJETIVO = 1024
NP_TOP_MARGIN = 64
# Desplazamiento vertical del bloque de Acceso manual.
# Positivo = baja el bloque; negativo = lo sube.
NP_VERTICAL_OFFSET = int(os.getenv("NP_VERTICAL_OFFSET", "90"))
# Separación vertical entre el input de matrícula y el teclado.
NP_INPUT_KEYS_GAP = int(os.getenv("NP_INPUT_KEYS_GAP", "100"))
# Separación entre teclas (manual/login). Puedes ajustarlo rápido sin tocar layout.
KB_GAP_X = int(os.getenv("KB_GAP_X", "4"))
KB_GAP_Y = int(os.getenv("KB_GAP_Y", "6"))
...
class FaceAccess(ctk.CTk):
    ...
    def __init__(self):
        ...
        self._app_w = min(APP_W_OBJETIVO, screen_w)
        self._app_h = min(APP_H_OBJETIVO, screen_h)
        self.geometry(f"{self._app_w}x{self._app_h}+0+0")
        self._safe_bottom = 56
        ...
        if self._app_w != APP_W_OBJETIVO or self._app_h != APP_H_OBJETIVO:
            print(
                f"[UI] Pantalla menor al objetivo {APP_W_OBJETIVO}x{APP_H_OBJETIVO}. "
                f"Usando {self._app_w}x{self._app_h}."
            )
        ...
        _APP_W         = self._app_w  # ancho real de la ventana
        _COLS_MAX      = 11           # columnas máximas (fila de números)
        _KB_PAD_X      = KB_GAP_X
        self._KB_BW    = (_APP_W - _KB_PAD_X * (_COLS_MAX + 1)) // _COLS_MAX  # ≈ 39px
        self._KB_BH    = 56           # altura generosa para dedos en 7"
        self._KB_FS    = 18           # fuente más legible en pantalla pequeña
        self._KB_PAD_X = _KB_PAD_X
        self._KB_PAD_Y = KB_GAP_Y
        self._KB_ACT_H = 56           # altura botones de acción (Espacio, OK, etc.)
        self._KB_SPC_W = max(_APP_W - 160, 220) # ancho botón Espacio, dejando hueco para Listo
    ...
    def _build_header(self):
        ...
        cv.create_text(16, 16, text="FA", fill=C_BG, font=("Helvetica", 15, "bold"))
        fn = ctk.CTkFrame(fl, fg_color="transparent")
        fn.pack(side="left")
        ctk.CTkLabel(fn, text="VisionID", font=("Helvetica", 20, "bold"), text_color=C_TXT).pack(anchor="w")
        ctk.CTkLabel(fn, text="Control de Acceso", font=("Helvetica", 15), text_color=C_TXT2).pack(anchor="w")
        ...
        self.lbl_fecha = ctk.CTkLabel(fr, text="", font=("Helvetica", 15), text_color=C_TXT2)
        self.lbl_fecha.pack(anchor="e")

    def _build_saludo(self):
        f = ctk.CTkFrame(self, fg_color=C_FOOT, corner_radius=0, height=H_SALUDO)
        f.pack(fill="x"); f.pack_propagate(False)
        self.lbl_saludo = ctk.CTkLabel(f, text="", font=("Helvetica", 13), text_color=C_TXT2)
        ...
        self.lbl_cnt_in = ctk.CTkLabel(fc, text="0 entradas", font=("Helvetica", 13), text_color=C_OK)
        self.lbl_cnt_in.pack(side="left", padx=(0, 10))
        ctk.CTkLabel(fc, text="●", font=("Helvetica", 8), text_color=C_TXT2).pack(side="left", padx=(0, 3))
        self.lbl_cnt_out = ctk.CTkLabel(fc, text="0 salidas", font=("Helvetica", 13), text_color=C_TXT2)
    ...
    def _build_video(self):
        ...
        self.lbl_badge = ctk.CTkLabel(self.frame_video, text="● Escaneando", font=("Helvetica", 13, "bold"), text_color=C_OK, fg_color=C_FRAME, corner_radius=0, padx=12, pady=4)
        self.lbl_badge.place(relx=1.0, rely=0.0, anchor="ne", x=-12, y=12)
        self.lbl_inst = ctk.CTkLabel(self.frame_video, text="", font=("Helvetica", 16, "bold"), text_color=C_OK, fg_color="#0D1E2D", corner_radius=20, padx=20, pady=8)
        self.btn_reg = ctk.CTkButton(self.frame_video, text="＋ Registrar", font=("Helvetica", 13, "bold"), width=110, height=36, fg_color=C_ADMIN, hover_color="#3C3489", text_color="white", corner_radius=0, command=self._abrir_login)
        self.btn_reg.place(relx=1.0, rely=1.0, anchor="se", x=-12, y=-self._safe_bottom)
    ...
    def _build_msg(self):
        ...
        self.lbl_msg_info   = ctk.CTkLabel(self.ov_msg, text="", font=("Helvetica", 13), text_color=C_TXT3)
    ...
    def _mostrar_msg(self, tipo, nombre="", matricula="", rol=""):
        dt_str = datetime.now().strftime(FORMATO_FECHA_HORA)
        if tipo == "exito":
            ...
            icono, titulo, sub, color = "⛔", TEXTO_ROSTRO_NO_REGISTRADO, TEXTO_SIN_COINCIDENCIA, C_ERROR
    ...
    def _build_numpad(self):
        ...
        ).pack(pady=(NP_TOP_MARGIN + NP_VERTICAL_OFFSET, 2))
        ...
        self._np_kb_frame.pack(padx=8, pady=(NP_INPUT_KEYS_GAP, 0), anchor="center")
        self._np_botones = {}
        ...
        ctk.CTkButton(
            fa, text="Cancelar",
            width=150, height=self._KB_ACT_H,
        ...
        ctk.CTkButton(
            fa, text="OK  ✓",
            width=220, height=self._KB_ACT_H,
    ...
    def _np_renderizar_teclado(self):
        ...
        BW  = self._KB_BW
        BH  = self._KB_BH
        PAD_X = self._KB_PAD_X
        PAD_Y = self._KB_PAD_Y
        FS  = self._KB_FS
        ...
        for col in range(max_cols):
            self._np_kb_frame.grid_columnconfigure(col, weight=1, uniform="np_keys")
    ...
    def _set_inst(self, t, c):
        if t:
            ...
            self.lbl_inst.configure(text="")
            self.lbl_inst.place_forget()
    ...
    def _update_clock(self):
        now = datetime.now()
        self.lbl_hora.configure(text=now.strftime(FORMATO_HORA))
        self.lbl_fecha.configure(text=now.strftime(FORMATO_FECHA))
        h = now.hour
